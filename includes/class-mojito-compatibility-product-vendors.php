...
class Mojito_Sinpe_Compatibility_Product_Vendors_Support
{
	...
	public function run() {
		...
		add_action('init', function () {
			add_action( WC_PRODUCT_VENDORS_TAXONOMY . '_add_form_fields', [ $this, 'add_vendor_custom_fields'] );
			add_action( WC_PRODUCT_VENDORS_TAXONOMY . '_edit_form_fields', [ $this, 'edit_vendor_custom_fields'], 10 );
			add_action( 'edited_' . WC_PRODUCT_VENDORS_TAXONOMY, [ $this, 'save_vendor_custom_fields'] );
			add_action( 'created_' . WC_PRODUCT_VENDORS_TAXONOMY, [ $this, 'save_vendor_custom_fields'] );
			add_action( 'wcpv_registration_form', [ $this, 'vendors_reg_custom_fields'] );
			add_action( 'wcpv_shortcode_registration_form_process', [ $this, 'vendors_reg_custom_fields_save'], 10, 2 );
		});
	...
	function add_vendor_custom_fields(){
		error_log('run : ' . __FUNCTION__ );
		wp_nonce_field( basename(__FILE__), 'vendor_custom_sinpe_nonce' );
		?>
		<div class="form-field">
			<label for="sinpe-number"><?php _e('Sinpe Number', 'mojito-sinpe'); ?></label>
	...
	function edit_vendor_custom_fields($term) {
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label for="sinpe-number"><?php _e('Sinpe Number', 'domain'); ?></label></th>
			<td>
	...
	function vendors_reg_custom_fields() {
		...
		<p class="form-row form-row-first">
			<label for="wcpv-sinpe-number"><?php esc_html_e('Sinpe Number', 'domain'); ?></label>
			<input type="text" class="input-text" name="sinpe-number" id="wcpv-sinpe-number" value="<?php if (!empty($_POST['sinpe-number'])) echo esc_attr(trim($_POST['sinpe-number'])); ?>" />
		</p>
		<?php
	}
	...
	function vendors_reg_custom_fields_save($args, $items) {
		...
		if ( isset( $items['sinpe-number'] ) && ! empty( $items['sinpe-number'] ) ) {
			$fb = sanitize_text_field($items['sinpe-number']);
			update_term_meta($term->term_id, 'sinpe-number', $fb);
		}
	}
}
